...
impl Map {
    ...
    pub fn shift_smart(&mut self, base_coord: [i8;3], n_sift: u8, axis: OpAxis, dir: bool) {
        assert!(n_sift != 0);
        if self.room_matrix.total() == 0 {return}
        let Some(&my_room) = self.room_matrix.get(base_coord) else {return};
        if !sift_range_big_enough(base_coord, n_sift-1, axis, dir) {return;}
        ...
        if !sift_vali((area_min,area_max), n_sift, axis, dir) {return;}
